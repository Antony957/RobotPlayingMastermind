...
try:
    from mastermind_interfaces.msg import Status, Code
except ImportError:
...
class VisionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.submit_code_topic = self.get_parameter("submit_code_topic").get_parameter_value().string_value
    ...
    def _status_callback(self, msg: Status) -> None:
        ...
        if msg.status == 3:
            if self.latest_image is not None:
                self.get_logger().info(f"Received Status 3 from '{msg.sender}'. Scanning...")
                self._process_frame()
            else:
                self.get_logger().warn("Received Status 3, but no image yet. Queuing scan.")
